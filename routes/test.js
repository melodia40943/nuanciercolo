  ...
  try {
    const result = await pool.query(`
      SELECT c.id, c.reference, c.hex, c.r, c.g, c.b,
             ...
             ARRAY(SELECT pc.pack_id FROM pack_couleurs pc WHERE pc.couleur_id = c.id) AS pack_ids
      FROM couleurs c
...
router.get('/api/marques-packs', async (req, res) => {
  try {
    ...
    `);
    const result = marques.rows.map(m => ({
      ...m,
      packs: packs.rows.filter(p => p.marque_id === m.id)
    }));
    res.json(result);
  } catch (err) {
    res.status(500).json({ error: 'Erreur serveur' });
